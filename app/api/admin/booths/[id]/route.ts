...
export async function GET( _req: Request, ctx: RouteContext<'/api/admin/booths/[id]'> ) {
  ...
  const r = await db.query( 'SELECT id, name, location, active, created_at, updated_at FROM app_booths WHERE id=$1', [id] )
...
export async function PATCH( request: Request, ctx: RouteContext<'/api/admin/booths/[id]'> ) {
  ...
  try {
    const body = await request.json(); if ( typeof body.name === 'string' ) name = body.name.trim(); if ( body.location !== undefined ) location = typeof body.location === 'string' ? body.location.trim() : null; if ( typeof body.active === 'boolean' ) active = body.active 
  } catch {
  ...
  const r = await db.query( `UPDATE app_booths SET ${sets.join( ',' )} WHERE id=$${i} RETURNING id,name,location,active,created_at,updated_at`, vals )
  if ( !r.rows.length ) return Response.json( { error : 'Not found' }, { status : 404 } )
